...
class Order {
  constructor (name, phone, gameId, done = false, id = null) {
    this.name = name
    this.phone = phone
    this.gameId = gameId
    this.done = done
...
export default {
  ...
  actions: {
    async createOrder ({commit}, {name, phone, gameId, ownerId}) {
      const order = new Order(name, phone, gameId)
      commit('clearError')
    ...
    async fetchOrders ({commit, getters}) {
      ...
      try {
        ...
        Object.keys(orders).forEach(key => {
          const o = orders[key]
          resultOrders.push(
            new Order(o.name, o.phone, o.gameId, o.done, key)
          )
        })
